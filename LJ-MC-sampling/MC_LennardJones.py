...
if len(input_grofile)>0:
    ...
    if postions.shape[0]!=num_particles: sys.exit("Error: input gro file has wrong number of particles")
    if (cell != cell2).any(): sys.exit("Error: input gro file has the wrong cell size")
else:
